...
class main extends UIfeeders
{
    ...
    public $status;
    public $appName = "Next Bus";
    public $author = "Hack Orient";
    public $dbname = "";
        ...
        try {
            ...
            if (count($subjects) > 0) {
                ...
                for ($count = 0; $count < count($subjects); $count++) {
                    ...
                    if($user->getUserType()=="agency" && ($action=="add"||$action=="view")&& $subjectTitle=="driver"){
                        echo "<li><a href='" . $action . "_article.php?article=$subjectId'>" . $subjectTitle . "</a></li>";
                    }else if($user->getUserType()=="traffic" && ($action=="view")&& $subjectTitle=="journey"){
                        echo "<li><a href='" . $action . "_article.php?article=$subjectId'>" . $subjectTitle . "</a></li>";
                    }else if(($action=="add")&& $subjectTitle=="journey"){
                        //XXX: Do nothing
                    }else if(($subjectTitle=="feature"||$subjectTitle=="slider"||$subjectTitle=="user")&&$user->getUserType()!="administrator"){
                        //XXX: Do nothing
                    }else if($user->getUserType()!="agency" &&$subjectTitle=="driver"){
                        //XXX: Do nothing
                    }else{
                        echo "<li><a href='" . $action . "_article.php?article=$subjectId'>" . $subjectTitle . "</a></li>";
                    }                    
...
class subject extends main
{
    ...
    public function isValid($title)
    {
        ...
        try {
            ...
            if (sizeof($check) != 0) {
                $status = false;
                error_log("Invalid subject title");
            }
        } catch (Exception $e) {
            $status = false;
            $this->status = "Error checking username." . $e;
